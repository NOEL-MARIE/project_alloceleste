...
"
      ...
      <div class="fixed md:top-[190px]  bottom-0 right-[100px]">
        <aside class="w-[492px] h-[989px] md:ml-96 rounded-xl shadow-lg bg-white px-5 flex flex-col">
          ...
          <div>
            <div
              class="relative flex items-center mt-2 text-sm text-red-500 cursor-default p- group"
            >
              *
...
function remplirCasierAvecProduit(produitId: string) {
  store.products.forEach((p) => {
    p.qty = p.id === produitId ? 24 : 0
  })
  store.save() // Sauvegarde dans localStorage si prévu dans ton store
}
